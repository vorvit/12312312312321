...
import { userPanelTemplate } from "./ui-templates/auth/user-panel";
import { fileBrowserTemplate } from "./ui-templates/auth/file-browser";
...
window.addEventListener('ifc-file-selected', async (event: CustomEvent) => {
  ...
  try {
    ...
    alert(`Error showing ${filename}: ${error.message}`);
  }
});
...
async function initializeAuth() {
  ...
  if (isAuthenticated) {
    // Load ALL user files in background
    ...
    if (user) {
      ...
      // Recreate user panel with new data
      const [newUserPanel] = BUI.Component.create(userPanelTemplate, {
        user, 
        isAuthenticated: true, 
        showUserMenu: false
      });
      userPanelContainer.replaceChild(newUserPanel, userPanel);
      userPanel = newUserPanel;
    }
    // Update file browser with ALL user files
    const [newFileBrowser] = BUI.Component.create(fileBrowserTemplate, {
      files: userFiles, 
      selectedFile: null, 
      searchTerm: "", 
      isLoading: false
    ...
    if (sidebarElement) {
      sidebarElement.replaceChild(newFileBrowser, fileBrowser);
      fileBrowser = newFileBrowser;
    }
  } else {
    console.log('Not authenticated');
    // Update components for unauthenticated state
    const [newUserPanel] = BUI.Component.create(userPanelTemplate, {
      user: null, 
      isAuthenticated: false, 
      showUserMenu: false
