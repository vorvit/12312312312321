...
import { getIFCFromCache, putIFCToCache, deleteIFCFromCache } from './cache';

function bytesToAsciiPrefix(bytes: Uint8Array, max: number = 64): string {
...
function isLikelyIFC(bytes: Uint8Array, filename: string): boolean {
  if (!bytes || bytes.length < 8) return false;
  const lower = filename.toLowerCase();
  const prefix = bytesToAsciiPrefix(bytes, 64);
  if (lower.endsWith('.ifczip')) {
    // ZIP magic: PK\x03\x04
    return bytes[0] === 0x50 && bytes[1] === 0x4b && bytes[2] === 0x03 && bytes[3] === 0x04;
  }
  if (lower.endsWith('.ifcxml')) {
    // Basic XML starts with '<'
    return prefix.trimStart().startsWith('<');
  }
  // .ifc text usually contains ISO-10303-21 header
...
export class AuthIntegration {
  ...
  async preloadAllUserIFCFiles(ifcLoader: any): Promise<void> {
    ...
    try {
      ...
      // Rehydrate path (prefer cache; fallback to network if cache invalid)
      if (filesAlreadyLoaded) {
        ...
        for (const file of ifcFiles) {
          try {
            ...
            if (!bytes || !isLikelyIFC(bytes, file.name)) {
              if (!bytes) console.warn(`Cache miss for ${file.name}`);
              else {
                console.warn(`Cache invalid for ${file.name}, deleting and refetching`);
                try { await deleteIFCFromCache(file.name, file.size, etag); } catch {}
              }
              const downloadUrl = `${this.authBaseUrl}/api/files/download/${file.name}?token=${this.authToken}`;
              const resp = await fetch(downloadUrl, { headers: { 'Content-Type': 'application/octet-stream' } });
              if (!resp.ok) { console.warn(`Refetch failed ${file.name}: ${resp.status}`); continue; }
              const buf = await resp.arrayBuffer();
              bytes = new Uint8Array(buf);
              if (!isLikelyIFC(bytes, file.name)) { console.warn(`Refetched bytes not IFC for ${file.name}`); continue; }
              try { await putIFCToCache(file.name, file.size, etag, bytes); } catch {}
            }
